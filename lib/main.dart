...
class _CustomsCalculatorState extends State<CustomsCalculator> {
  ...
  void initState() {
    ...
    _goodsValueController.addListener(_onGoodsValueChange); // Add listener to the TextField
  }

  @override
  void dispose() {
    _goodsValueController.removeListener(_onGoodsValueChange); // Remove listener when disposing
    _goodsValueController.dispose();
    super.dispose();
  }
  void _onGoodsValueChange() {
    if (_goodsValueController.text.isEmpty) {
      setState(() {
        _customsDuty = 0.0;
        _vat = 0.0;
        _totalFees = 0.0;
      ...
      _calculateCustomsFees();
    }
  }

  Future<void> _calculateCustomsFees() async {
    ...
    setState(() {
      _isLoading = true;
      _errorText = null; // Reset any previous errors
    });

    try {
      double exchangeRate = await fetchExchangeRate(_selectedCurrency);
                ...
                ConstrainedBox(
                  ...
                  child: DropdownButton<String>(
                    ...
                    onChanged: (String? newValue) {
                      ...
                      _calculateCustomsFees(); // Recalculate fees when currency changes
                    },
                    items: ['USD', 'EUR', 'GBP'].map<DropdownMenuItem<String>>((String value) {
                      return DropdownMenuItem<String>(
                        ...
                        child: Row(
                          children: <Widget>[
                            Image.asset('assets/images/$value.png', width: 24, height: 24),
                    ...
                    rows: _exchangeRates.entries.map((entry) => DataRow(
                      cells: [
                        DataCell(Row(children: [
                          Image.asset('assets/images/${entry.key}.png', width: 20),
                          Text("  1 ${entry.key}"),
                        ])),
                        DataCell(Row(children: [
